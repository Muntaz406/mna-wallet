...
// 1. DATA KOIN & HARGA DEFAULT (Agar tidak blank jika API mati)
const cryptoDB = [
    ...
    { id: 'dogwifhat', symbol: 'WIF', name: 'Dogwifhat', defaultPrice: 2.80, img: 'https://assets.coingecko.com/coins/images/33566/standard/dogwifhat.jpg', tvSymbol: 'BINANCE:WIFUSDT' },
    { id: 'bonk', symbol: 'BONK', name: 'Bonk', defaultPrice: 0.000021, img: 'https://assets.coingecko.com/coins/images/28600/standard/bonk.jpg', tvSymbol: 'BINANCE:BONKUSDT' },
    { id: 'floki', symbol: 'FLOKI', name: 'Floki', defaultPrice: 0.0002, img: 'https://assets.coingecko.com/coins/images/16746/standard/FLOKI.png', tvSymbol: 'BINANCE:FLOKIUSDT' }
];
...
// 2. INISIALISASI HARGA FALLBACK (Menjamin 100% selalu muncul)
...
// 3. FETCH LIVE PRICES (Update dari API jika jalan)
async function fetchLivePrices() {
    try {
        ...
        console.warn("Using Fallback Prices. Error:", error);
    } finally {
        // Render wajib dipanggil, baik sukses atau gagal API
        renderHomeAssets();
        renderMarketAssets();
    }
}

// --- RENDER KOIN (Aman 100%) ---
...
// Template Koin HTML + Auto-Generate Logo Error Handler
function createCoinCard(coin, fiatValue, amount, isMarket = false) {
    ...
    // Fallback Logo jika internet putus/link gambar rusak
    ...
    <div ${onClickAttr}>
        <div class="flex items-center gap-4">
            <img src="${coin.img}" alt="${coin.symbol}" class="coin-logo" ${fallbackImage}>
            <div><h4 class="font-bold text-[15px] text-white">${coin.symbol}</h4><p class="text-[11px] text-gray-400">${coin.name}</p></div>
        </div>
        <div class="text-right">${rightSide}</div>
    </div>`;
}
...
function loadSaveData() {
    ...
    initFallbackPrices(); // Panggil Default Dulu
    updateProfileImages();
    updateUsernameUI();
    renderHomeAssets(); // Render Pertama kali pakai Default
    renderMarketAssets();
    fetchLivePrices(); // Baru tarik API diam-diam
}
...
function updateUsernameUI() {
    ...
    if (headEl) headEl.innerText = uName;
    if (modEl) modEl.innerText = uName;
    if (pinEl) pinEl.innerText = `Welcome Back, ${uName.split(' ')[0]}`;
}
...
function changeChart(symbol) {
    ...
    new TradingView.widget({
        ...
        "backgroundColor": "rgba(10, 10, 12, 0)", "gridColor": "rgba(255, 255, 255, 0.05)",
        "hide_top_toolbar": true, "hide_legend": true, "save_image": false, "container_id": "tradingview_mna"
    });
    window.scrollTo({ top: 0, behavior: 'smooth' });
}

function updateProfileImages() {
    ['pinProfileImg', 'headerProfileImg', 'modalProfileImg'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.src = appState.profilePic;
    });
}
...
document.getElementById('profileUpload').addEventListener('change', function(event) {
    ...
    if (file) {
        ...
        reader.onload = function(e) { appState.profilePic = e.target.result; updateProfileImages(); saveData(); showToast("Profile Picture Saved!"); };
...
function handleAction(type) {
    ...
    if(type==='send') body.innerHTML = `<input type="text" placeholder="Wallet Address" class="w-full bg-gray-900 border border-gray-700 rounded-xl p-3 mb-3 text-white"><input type="number" placeholder="Amount" class="w-full bg-gray-900 border border-gray-700 rounded-xl p-3 text-white mb-4"><button onclick="showToast('Send Complete'); toggleModal('actionModal')" class="w-full bg-blue-600 text-white font-bold py-3 rounded-xl">Send</button>`;
...
function executeTopUp() { const amt = parseFloat(document.getElementById('topupAmt').value); if(amt>0) { appState.totalBalance+=amt; saveData(); renderHomeAssets(); toggleModal('actionModal'); showToast(`Top Up $${amt} Success!`); } }
